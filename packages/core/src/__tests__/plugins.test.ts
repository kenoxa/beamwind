import type { Instance, Injector, InlinePlugin } from '../types'

import { createInstance, virtualInjector, noprefix, apply, strict } from '..'
...
test('plugin can return new tokens to parse using `apply`', () => {
  ...
  expect(() => instance.bw('btn-unknown-color')).toThrow(
    `[btn-unknown-color] Plugin "btn" had no result`,
  )
})
